...
def solicitar_sensores(comandos, modo='NORMAL', frecuencia=None):
    ...
    if modo == 'NORMAL':
       ...
       print("[DEBUG] he psasdo directo a LC")
       ...
       while not detener_LC:
           # en LC, aumentamos el timeout para que espere el primer dato
           timeout = int(frecuencia) + 4  # ejemplo: si freq=5s → espera 7s máximo
        ...
        detener_LC = True  # ← Aquí se detiene el bucle LC
        comandos = "STOP#"
        GPIO.output(M0, GPIO.LOW)
        GPIO.output(M1, GPIO.LOW)
        time.sleep(0.05)
        ser.write(b'STOP#')
        print(f"[TX] Enviando comando al Arduino: {b'STOP#'}")
