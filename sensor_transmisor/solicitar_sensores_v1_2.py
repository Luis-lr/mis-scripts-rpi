...
def esperar_respuesta(mensaje_esperado, timeout=0.1):  # 20 ms de espera tras cada WAKE#
    ser.flushInput()
    start = time.time()
    buffer = ""
...
def intentos_wake(duracion_ventana=6, intervalo_intento=0.1):
    ...
    while time.time() - inicio < duracion_ventana:
        ...
        if esperar_respuesta("READY#", timeout=0.200):  # espera 200 ms tras cada intento
...
def solicitar_sensores(comandos, modo='NORMAL', frecuencia=None):
    # Aquí preparas el mensaje según el modo
    if modo == 'NORMAL':
        comando_str = ''.join(comandos) + '#'
        intentos_wake()
    elif modo == 'LC':
        if not frecuencia or len(comandos) != 1:
            return {'error': 'Modo LC requiere frecuencia y un solo comando'}
        # Sin comas
    ...
    #return {'status': 'Comando preparado', 'comando': comando_str}
